...
/**
 * Hook for managing filter chips state (active filters, dropdown open state).
 * 
 * Solves race condition when closing one dropdown and opening another by
 * providing `ignoreChipRefs` to FilterDropdown components.
 * 
 * @example
 * ```tsx
 ...
 * ```
 */
export function useFilterChips({ chipRefs, onResetFilter }: UseFilterChipsParams = {}) {
